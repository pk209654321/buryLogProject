...
object ProcessingMBGiftOrder {
        ...
        it.foreach(line => {
          ...
          if (typeStr.equals("insert") || typeStr.equals("update") || typeStr.equals("delete")) {
            ...
            if (jsonData != null) {
              ...
              ProcessingMBOrderData.getRightTimeByName(jsonData, "close_time")
            } else {
  ...
  def doProcessingMBData2(session: KuduSession, jsonObject: JSONObject, db_s: String, tb_s: String, db_t: String, tb_t: String, kuduTb: String): Unit = {
    var jsonData = new JSONObject();
